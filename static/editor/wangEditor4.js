...
DomElement.prototype = {
    ...
    findClass: function findClass(className) {
        ...
        if (elem.className) {
            ...
            if(className){
                arr = arr.filter(function (item) {
                    return className.test(item.trim())
                });
            }
            return arr
        }
...
Code.prototype = {
    ...
    _createPanel: function _createPanel(elem) {
        var _this = this;

        // value - 要编辑的内容
        var type = elem ? 'edit' : 'new';
        var value = elem ? elem.html() : '';
        console.log(elem)
        var lang = elem ? elem.findClass(/language-\w+/).toString().replace(/language-(\w+)/,'$1').replace(/^default$/,'') : ''
        var line = elem && elem.findClass() ? "checked" : ""
        var textId = getRandom('texxt');
        var lineId = getRandom('line'); 
        var langId = getRandom('land'); 
        var btnId = getRandom('btn');
        //var oldlang = this.findClass(/language-\w+/).toString()
        ...
        var panel = new Panel(this, {
            ...
            tabs: [{
                ...
                title: '插入代码',
                // 模板
                tpl: '<div>\n   <textarea id="' + textId + '" style="height:145px;;">' + value + '</textarea>\n                        <div class="w-e-button-container">\n    <input id="'+lineId+'"type="checkbox" '+line+'/>\u663e\u793a\u884c\u53f7   <input id="' + langId + '" type="text" value="'+lang+'" placeholder="\u9ad8\u4eae\u8bed\u6cd5"/>                       <button id="' + btnId + '" class="right">\u63D2\u5165</button>\n                        </div>\n                    <div>',
                // 事件绑定
                events: [
                // 插入代码
                {
                    ...
                    fn: function fn() {
                        ...
                        var line = $line[0].checked
                        text = replaceHtmlSymbol(text);
                        if (type === 'new') {
                            // 新插入
                            _this._insertCode(text,lang,line);
                        } else {
                            // 编辑更新
                            _this._updateCode(text,lang,line);
                        }
    ...
    // 插入代码
    _insertCode: function _insertCode(value,lang,line) {
        var editor = this.editor;
        lang = 'language-'+ (lang || 'default')
        line = line ? "line-numbers " : ""
        editor.cmd.do('insertHTML', '<pre><code class="'+ line + lang +'">' + value + '</code></pre><p><br></p>');
    },

    // 更新代码
    _updateCode: function _updateCode(value,lang,line) {
        var editor = this.editor;
        var $selectionELem = editor.selection.getSelectionContainerElem();
        if (!$selectionELem) {
            return;
        }
        var oldlang = $selectionELem.findClass(/language-\w+/).toString()
        lang = 'language-'+ (lang || 'default')
        if(oldlang!==lang){
            $selectionELem.removeClass(oldlang)
            $selectionELem.addClass(lang)
        }
        console.log(line)
        if(line){
            $selectionELem.addClass("line-numbers")
        }else{
            $selectionELem.removeClass("line-numbers")
        }
...
// 返回
var index = window.Editor || Editor;
