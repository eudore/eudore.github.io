...
Code.prototype = {
    ...
    _createPanel: function _createPanel(elem) {
        ...
        var panel = new Panel(this, {
            ...
            tabs: [{
                ...
                title: '插入代码',
                // 模板
                tpl: '<div>\n                        <textarea id="' + textId + '" style="height:145px;;">' + value + '</textarea>\n                        <div class="w-e-button-container">\n     <input id="' + langId + '" type="text" value="'+lang+'" placeholder="\u9ad8\u4eae\u8bed\u6cd5"/>                       <button id="' + btnId + '" class="right">\u63D2\u5165</button>\n                        </div>\n                    <div>',
                // 事件绑定
                events: [
                // 插入代码
                {
                    ...
                    fn: function fn() {
                        ...
                        if (type === 'new') {
                            // 新插入
                            _this._insertCode(text,lang);
                        } else {
                            // 编辑更新
                            _this._updateCode(text,lang);
                        }
    ...
    // 插入代码
    _insertCode: function _insertCode(value,lang) {
        var editor = this.editor;
        lang = 'language-'+ (lang || 'default')
        editor.cmd.do('insertHTML', '<pre><code class="line-numbers ' + lang +'">' + value + '</code></pre><p><br></p>');
    },

    // 更新代码
    _updateCode: function _updateCode(value,lang) {
        var editor = this.editor;
        var $selectionELem = editor.selection.getSelectionContainerElem();
        if (!$selectionELem) {
            return;
                ...
                if (xhr.readyState === 4) {
                    ...
                    if ((typeof result === 'undefined' ? 'undefined' : _typeof(result)) !== 'object') {
                        try {
                            ...
                            _this3._alert('上传图片失败', '上传图片返回结果错误，返回结���是: ' + result);
                            return;
                        }
                    }
