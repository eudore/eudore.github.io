...
static ngx_flag_t ngx_http_oauth(u_char *token,ngx_int_t type);
...
static ngx_int_t ngx_http_oauth_header_filter(ngx_http_request_t *r)
{
    ...
    head = r->uri.data;
    last= head + r->uri.len;
    head = ngx_strlchr(head,last,'/');
    head = ngx_strlchr(head+1,last,'/') + 1;
    last = ngx_strlchr(head,last,'/');

    u_char key[48]={""};
    ngx_str_t name = ngx_string("token");
    ngx_str_t token = ngx_null_string;
#ifdef NGX_DEBUG
    ngx_log_error(NGX_LOG_DEBUG, r->connection->log, 0, "head: \"%s\"",head); 
    ngx_log_error(NGX_LOG_DEBUG, r->connection->log, 0, "type: \"%i\"", conf->type); 
    ngx_log_error(NGX_LOG_DEBUG, r->connection->log, 0, "uri: \"%V\"", r->uri); 
#endif
/*
    if(NGX_OK == ngx_http_parse_multi_header_lines(&r->headers_in.cookies, &name, &token)){
        ngx_memcpy(key,token.data,token.len);
        ngx_memcpy(key+32,head,last-head);
        ngx_log_error(NGX_LOG_DEBUG, r->connection->log, 0, "key: \"%s\"",key); 
        ngx_log_error(NGX_LOG_DEBUG, r->connection->log, 0, "key: \"%s\"",key); 
#ifdef NGX_DEBUG
        ngx_log_error(NGX_LOG_DEBUG, r->connection->log, 0, "cook %V: \"%V\"",&name, &token); 
#endif
        if(ngx_http_oauth(key,conf->type)){
            return ngx_http_next_header_filter(r);
        }
    }       */
...
static ngx_flag_t ngx_http_oauth(u_char *token,ngx_int_t type){
    ...
    value = memcached_get(memc,(char*)token,strlen((char*)token),NULL,NULL,&rc);
    memcached_free(memc);
        
    if(rc == MEMCACHED_SUCCESS){
        return type == (atoi(value)&type);
    }
    return false;
}